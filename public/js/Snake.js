export default class Snake {
    constructor(scene) {
        this.scene = scene;
        this.lastMoveTime = 0;
        this.moveInterval = 100; // In milliseconds
        this.tileSize = 16;
        this.body = [];
    ...
    move() {
        ...
        if (this.apple.x === x && this.apple.y === y) {
            // eaten the apple, increase body size
        ...
        if (tail.filter(s => s.x === this.body[0].x
            && s.y === this.body[0].y).length > 0) {
                this.scene.scene.restart();
            }
    }
 }